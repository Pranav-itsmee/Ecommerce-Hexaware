...
public class OrderDAOImpl implements OrderDAO {
    ...
    public void showAllOrderItems() {
        String query = "SELECT oi.*, p.product_name FROM order_items oi " +
        ...
        try (Connection connection = ConnectionHelper.getConnection();
            ...
            while (resultSet.next()) {
                ...
                System.out.println("Order Item ID: " + orderItemId + ", Order ID: " + orderId +
                        ", Product: " + productName + ", Quantity: " + quantity + ", Price: " + price);
            }
        } catch (SQLException | ClassNotFoundException e) {
            e.printStackTrace();
        }
    ...
    public void showOrderItemsByProductId(int productId) {
        String query = "SELECT oi.*, o.customer_id FROM order_items oi " +
                       "JOIN orders o ON oi.order_id = o.order_id WHERE oi.product_id = ?";
        try (Connection connection = ConnectionHelper.getConnection();
             PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            while (resultSet.next()) {
                ...
                System.out.println("Order Item ID: " + orderItemId + ", Order ID: " + orderId +
                        ", Customer ID: " + customerId + ", Quantity: " + quantity + ", Price: " + price);
            }
        } catch (SQLException | ClassNotFoundException e) {
            e.printStackTrace();
        }
